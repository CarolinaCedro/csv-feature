...
import maxxsoft.conversorCSV.aplication.entities.enums.HeadersSummaryValids;

import java.io.IOException;
import java.util.LinkedList;
import java.util.List;

public class HeadersSummaryValidsDesserializer extends JsonDeserializer<List<Collum>> {
    @Override
    public List<Collum> deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JacksonException {
        ...
        node.forEach(it -> results.add(HeadersSummaryValids.from(it.asText())));
        return results;
    }
